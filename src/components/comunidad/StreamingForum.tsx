...
export default function StreamingForum() {
  ...
  const getUserRoleLabel = () => {
    if (isAdmin) return "Admin • Dirección";
    if (isStaff) return `Staff • ${profile?.department || "Elenco"}`;
    return "Comunidad de Fiesta Pagana";
  };
  ...
  const handleAddReply = (threadId: string) => {
    ...
    const authorName = profile?.full_name || (user?.email ? user.email.split("@")[0] : "Espectador de la Comunidad");
                    ...
                    <div className="flex items-center gap-3">
                      {thread.avatarUrl ? (
                        <img
                          ...
                          alt={thread.author}
                          className="w-10 h-10 rounded-full object-cover border border-[#fabc4d]/50"
                        />
                      ) : (
                        <div className="w-10 h-10 rounded-full bg-[#9e2a2b]/30 border border-[#fabc4d]/50 flex items-center justify-center text-[#fabc4d] font-bold">
                    ...
                    <div className="bg-[#0b0b0e] border-t border-[#58413f]/40 p-6 flex flex-col gap-4">
                      ...
                      <div className="pt-2 flex items-center gap-2">
                        <input
                          ...
                          placeholder={
                            user
                              ? `Comentar como ${profile?.full_name || user.email?.split("@")[0]}...`
                              : "Escribí tu reflexión o aporte sobre esta escena..."
                          }
              ...
              <form onSubmit={handleCreateThread} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="w-full bg-[#0b0b0e] border border-[#58413f] rounded-xl px-4 py-2.5 text-xs text-[#efbf67]">
                      {profile?.full_name || (user?.email ? user.email.split("@")[0] : "Espectador de la Comunidad")} ({getUserRoleLabel()})
                    </div>
                  </div>
                </div>
